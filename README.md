# LegalTech AI 법률 상담사 & 정산 전문가 프로젝트

## 1. 과제 개요 - 프로젝트 기획 배경 및 핵심 내용

### 프로젝트를 기획한 이유

현대 사회에서 법률 정보와 정산 업무 지식에 대한 접근성은 매우 중요하지만, 일반인이 신뢰할 수 있는 법률 상담을 받거나 복잡한 정산 업무 지식을 얻기에는 시간적·경제적 제약이 큽니다. 본 프로젝트는 최신 AI 기술을 활용하여 누구나 쉽고 빠르게 법률 정보와 정산 업무 지식을 얻을 수 있는 서비스를 제공하고자 기획되었습니다.

### 해결하고자 하는 문제

- 법률 문서(법령, 판례 등)의 방대한 양과 복잡성으로 인해 비전문가가 필요한 정보를 찾기 어려움
- 정산 업무 관련 문서와 지식의 복잡성으로 인한 업무 효율성 저하
...
- 법률 PDF 문서와 Confluence 문서에서 텍스트를 추출·청킹하고, Azure OpenAI 임베딩을 통해 ChromaDB 벡터스토어에 저장
- RAG Tool(search_law, search_settlement_info)을 통해 사용자의 질문과 유사한 법률 조항/정산 업무 정보를 벡터DB에서 검색
- 검색된 정보를 답변에 명확히 인용하여 신뢰성 있는 결과 제공
- 전문 법률 상담사/정산 전문가 프롬프트와 Tool Agent(create_react_agent)로 논리적·근거 중심의 답변 생성
- Streamlit UI와 연동하여 대화 기록, 참조 문서, 진행상태 등 실시간 피드백 제공

### 기대하는 사용자 경험 및 주요 특징

- 누구나 쉽고 빠르게 법률 상담 및 정산 업무 지식 획득 가능 (24/7)
- 답변마다 관련 법률 조항/정산 업무 문서/출처를 명확히 인용하여 신뢰성 제공
- 최신 법률 PDF 파일과 Confluence 문서를 손쉽게 갱신/추가 가능
...
- 전문 법률 상담사/정산 전문가 역할, 사고과정, 인용 방식 등 프롬프트 설계
- 사용자의 질문을 분석하고, RAG Tool을 통해 관련 정보를 검색하도록 유도
- 답변 생성 시 법률적 근거(조문, 판례 등) 또는 정산 업무 근거(절차, 가이드 등) 명확히 인용

### 2) Azure OpenAI 활용

- Azure OpenAI의 GPT-4/3.5 등 LLM API를 활용한 자연어 이해 및 생성
- Azure OpenAI Embedding API로 법률 문서 및 정산 문서 임베딩 및 벡터화
- API Key, Endpoint, Model Name 등 환경변수 기반 안전한 연동

### 3) RAG (Retrieval-Augmented Generation)

- PyPDFLoader, PyMuPDFLoader로 PDF 텍스트 추출 및 청킹
- Confluence JSON 데이터를 BeautifulSoup으로 HTML 파싱하여 텍스트 추출
- Azure OpenAI 임베딩 → ChromaDB 벡터스토어 저장/검색
- 유사도 기반 리트리버(get_legal_retriever, get_settlement_retriever)로 관련 정보 검색
- RAG Tool을 통해 LLM 답변 생성 시 근거 문서 자동 인용

### 4) Streamlit 및 서비스 개발/패키징

- Streamlit 기반 대화형 챗봇 UI 구현 (st.chat_input, st.chat_message 등)
- 질문 유형 선택 기능 (법률 상담 / 정산 업무)
...
- 실제 법률 PDF와 Confluence 문서를 다루고, 프롬프트 엔지니어링, 벡터DB, 상태관리 등 다양한 기술이 유기적으로 연결되는 경험이 매우 인상적이었습니다.
...
- **분리된 벡터스토어**: 법률 문서와 정산 문서를 별도의 ChromaDB로 관리
- **자동 문서 로딩**: PDF와 Confluence JSON 파일을 자동으로 로드
- **HTML 파싱**: Confluence HTML 내용을 텍스트로 변환
- **메타데이터 보존**: 페이지 제목, URL, 출처 정보 등 메타데이터 유지
...
chroma_db/
├── legal/          # 법률 관련 벡터스토어
└── settlement/     # 정산 관련 벡터스토어
```

### 4. 사용법

- **법률 상담**: `get_legal_retriever()` 사용
- **정산 업무**: `get_settlement_retriever()` 사용
- 각각의 리트리버는 해당 도메인의 문서만 검색 